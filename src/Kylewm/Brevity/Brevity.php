...
class Brevity
{
    ...
    function __construct() {
      ...
      $this->linkifyRegex = sprintf('/(?:%s)?%s(?:%s|\b)/i', $scheme, $host, $path);
      $this->domainRegex = sprintf('/(?:%s)?%s/i', $scheme, $host);
    }
    ...
    private function weightedCharLengthAt($str, $index)
    {
      ...
      return $weight / self::WEIGHTS['scale'];
    }
    ...
    private function truncateToNearestWord($text, $length)
    {
        ...
        for ($jj = mb_strlen($text)-1 ; $jj >= 0 ; --$jj) {
            if (mb_strstr($delimiters, $text[$jj])) {
                $trunc = rtrim(substr($text, 0, $jj), $delimiters);
