...
class Brevity
{
    ...
    private $targetLength = 280;

    private $linkLength = 23;

    function __construct() {
      $scheme = '(?:http|https|file|irc):\/{1,3}';
      $host = '(?:[a-z0-9][a-z0-9\-]*\.)+(?:' . implode('|', self::TLDS) . ')(?::\d{2,6})?';
      $path = '\/[\w\/.\-_~.;:%?!@$#&()=+]*';

      $this->linkifyRegex = sprintf('/(?:%s)?%s(?:%s|\b)/iu', $scheme, $host, $path);
    }
    ...
    {
        preg_match_all($this->linkifyRegex, $text, $links);
        $splits = preg_split($this->linkifyRegex, $text);
        $links = $links[0];
        ...
        for ($ii = 0 ; $ii < count($links) ; ++$ii) {
            ...
            while ($jj >= 0 && mb_strstr(".!?,;:)", $link[$jj])
                ...
                --$jj;
                $links[$ii] = mb_substr ($link, 0, $jj+1);
                $splits[$ii+1] = mb_substr ($link, $jj+1) . $splits[$ii+1];
                $link = $links[$ii];
            }
    ...
    function tweetLength($text) {
        return $this->totalLength($this->tokenize($text, true));
    }
    ...
    private function tokenLength($token)
    {
        ...
        return $this->textTokenLength($token->content);
    }

    private function textTokenLength($val)
    {
        $total  = 0;
        for ($ii = 0 ; $ii < mb_strlen($val) ; $ii++) {
            $total += $this->weightedCharLengthAt($val, $ii);
        }
        return $total;
    }

    private function weightedCharLengthAt($str, $index)
    {
      $char = mb_substr($str, $index, 1, 'UTF-8');
      $point = 0;
      if (mb_check_encoding($char, 'UTF-8')) {
        $ret = mb_convert_encoding($char, 'UTF-32BE', 'UTF-8');
        $point = hexdec(bin2hex($ret));
      }
      $weight = self::WEIGHTS['defaultWeight'];
      foreach (self::WEIGHTS['ranges'] as $range) {
          if ($point >= $range['start'] && $point <= $range['end']) {
            $weight = $range['weight'];
            break;
          }
      }
      return floor($weight / self::WEIGHTS['scale']);
    }
    ...
    private function truncateToNearestWord($text, $length)
    {
        $delimiters = ",.;: \t\r\n";
        $text = rtrim($text);
        if ($this->textTokenLength($text) <= $length) {
            return $text;
        }
        // walk backwards until we find a delimiter
        for ($jj = mb_strlen($text)-1 ; $jj >= 0 ; --$jj) {
            if (mb_strstr($delimiters, $text[$jj])) {
                $trunc = rtrim(mb_substr ($text, 0, $jj), $delimiters);
                if ($this->textTokenLength($trunc) <= $length) {
                    return $trunc;
                }
            }
        }
