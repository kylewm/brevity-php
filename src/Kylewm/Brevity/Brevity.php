...
class Brevity
{
    ...
    const LINKIFY_RE = '/(?:(?:http|https|file|irc):\/{1,3})?(?:[a-z0-9][a-z0-9\-]*\.)+[a-z]{2,}(?::\d{2,6})?(?:(?:\/[\w\/.\-_~.;:%?!@$#&()=+]*)|\b)/i';
    const DOMAIN_RE  = '/(?:(?:http|https|file|irc):\/{1,3})?((?:[a-z0-9][a-z0-9\-]*\.)+)([a-z]{2,})/i';
    ...
    private function hasValidTld($link)
    {
        if (preg_match(self::DOMAIN_RE, $link, $m)) {
            return in_array(mb_strtolower($m[2]), self::TLDS);
        }
        return false;
    }
